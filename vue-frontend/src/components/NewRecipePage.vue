...
<template>
  <div class="content-container">
    ...
    <div class="form-container">
      ...
      <v-select v-model="recipe.difficulty" :items="['Easy', 'Intermediate', 'Hard']" label="Difficulty" required></v-select>
    ...
    async uploadToDatabase() {
      ...
      try {
        ...
        for (let key in this.recipe) {
          // Check if the value corresponding to the key is an empty string
          if (key !== 'notes' && this.recipe[key] === '') {
            this.$root.snackbar.display(false, `${key.charAt(0).toUpperCase() + key.slice(1)} cannot be empty`);
            return;
          }
        }

        if (this.recipe.time === null || parseInt(this.recipe.time) <= 0) {
          this.$root.snackbar.display(false, 'Invalid time');
          return;
        }

        const ingredientsArray = this.recipe.ingredients.split('\n').filter(item => item !== '');
        const instructionsArray = this.recipe.instructions.split('\n').filter(item => item !== '');
        const notesArray = this.recipe.notes.split('\n').filter(item => item !== '');
